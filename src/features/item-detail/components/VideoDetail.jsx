    ...
    <DetailWrapper>
        ...
        <div className="text-center">
          ...
          <div className="flex flex-wrap gap-6 justify-center">
            <div className="text-center">
              ...
              <span className="mt-1 block text-xl font-bold bg-gradient-to-r from-purple-500 to-indigo-500 text-transparent bg-clip-text">🎬 영상</span>
            </div>
            <div className="text-center">
            ...
            {item.duration && (
              <div className="text-center">
                <span className="text-sm font-medium text-gray-500">재생 시간</span>
                <span className="mt-1 block text-base font-semibold text-gray-700">⏱️ {item.duration}</span>
              </div>
            )}
      ...
      <main>
        <section className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4 pb-2 border-b-2 border-white/30">영상</h2>
          <div className="text-center mb-6">
            {item.videoUrl && !videoError ? (
              <video 
                controls 
                className="max-w-full w-[800px] h-[450px] rounded-xl shadow-lg mx-auto"
              ...
              <div className="w-full max-w-[800px] h-[450px] bg-gray-800/95 rounded-xl flex flex-col items-center justify-center text-white text-lg mx-auto backdrop-blur-md border-2 border-white/20 shadow-lg transition-all duration-300 hover:bg-gray-800 hover:border-purple-300/50 hover:shadow-xl">
                <span className="text-5xl mb-4">🎬</span>
                <p>{videoError ? '영상을 불러올 수 없습니다' : '영상이 생성 중입니다'}</p>
                {item.duration && <small className="text-sm text-gray-400 mt-2">예상 길이: {item.duration}</small>}
              </div>
            )}
          </div>
        </section>
        ...
        <section className="mb-8">
          <h2 className="text-2xl font-semibold text-gray-800 mb-4 pb-2 border-b-2 border-white/30">영상 정보</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-gray-50/30 rounded-lg border border-white/40 shadow-sm">
              <strong className="block text-gray-700 mb-1">제목:</strong> 
              <p className="text-gray-800">{item.title}</p>
            </div>
            <div className="p-4 bg-gray-50/30 rounded-lg border border-white/40 shadow-sm">
              <strong className="block text-gray-700 mb-1">설명:</strong> 
              <p className="text-gray-800">{item.description}</p>
            </div>
            {item.duration && (
              <div className="p-4 bg-gray-50/30 rounded-lg border border-white/40 shadow-sm">
                <strong className="block text-gray-700 mb-1">재생 시간:</strong> 
                <p className="text-gray-800">{item.duration}</p>
              </div>
            )}
            <div className="p-4 bg-gray-50/30 rounded-lg border border-white/40 shadow-sm">
              <strong className="block text-gray-700 mb-1">생성일:</strong> 
              <p className="text-gray-800">{item.createdAt}</p>
            </div>
            <div className="p-4 bg-gray-50/30 rounded-lg border border-white/40 shadow-sm">
              <strong className="block text-gray-700 mb-1">AI 생성:</strong> 
              <p className="text-gray-800">{item.aiGenerated ? '예' : '아니오'}</p>
            </div>
          </div>
        </section>
